...
Test::Test()
    : dump_file("log/match_" + to_string(time(NULL)) + ".txt"),
      fout(dump_file, ios::app), games(0), wins(0) {}
Test::~Test() { fout.close(); }

void Test::run_test() {
  if (AI_MATCH)
    match_bet_AIs();
  if (AI_QUIZ)
    solve_quiz(QUIZ_NUM, PUT_BOARD);
}
...
bool Test::match_play(int nth) {
  ...
  if (b.get_player(b.whose_turn()) == MARIAI)
    return true;
  else
    return false;
}
...
void Test::print_param_info() {
  fout << setw(15) << "PLAYOUTS"
       << " | " << PLAYOUTS << endl;
  fout << setw(15) << "BRANCHING"
       << " | " << BRANCHING << endl;
  fout << setw(15) << "UCB_C"
       << " | " << UCB_C << endl;
  fout << setw(15) << "UCB_POW"
       << " | " << UCB_POW << endl;
  cout << setw(15) << "PLAYOUTS"
       << " | " << PLAYOUTS << endl;
  cout << setw(15) << "BRANCHING"
       << " | " << BRANCHING << endl;
  cout << setw(15) << "UCB_C"
       << " | " << UCB_C << endl;
  cout << setw(15) << "UCB_POW"
       << " | " << UCB_POW << endl;
}
...
void Test::print_per_game(Board &b, int &lines, int &pass, int nth,
                          int start_at) {
  string nickname;
  string turn;
  if (b.whose_turn() == BLACK) {
  ...
  rewind_stdout(lines, pass);
  cout << setw(3) << nth << " | ";
  cout << turn << nickname << " wins, " << b.moves << " moves. " << flush;
  cout << setprecision(2) << fixed << (time(NULL) - start_at) / 60. << " mins."
       << endl;

  fout << setw(3) << nth << " | ";
  fout << turn << nickname << " wins, " << b.moves << " moves. " << flush;
  fout << setprecision(2) << fixed << (time(NULL) - start_at) / 60. << " mins."
       << endl;
}
...
void Test::solve_quiz(int quizN, bool put_board) {
  ...
  m.gen_candy(b);
  b.candy = m.candy;
  cout << "Best move: " << setw(2) << get<0>(g) << " " << setw(2) << get<1>(g)
       << endl;
  if (put_board)
    b.print_board(true);
}
