...
bool Mariai::backpropagation(Node* node, Node* roof, Stone turn) {
    while ( node != NULL ) {
        node->visit++;
        // fast-decision
        if ( node != roof && node->visit > EARLY_CUT ) return true;
...
void Mariai::gen_candy(Board &b) {
    ...
    candy.clear();
    analyze_pattern(b, 1);
    if ( !candy.size() ) analyze_pattern(b, 0);
}
...
void Mariai::find_pattern_inline(Board &b, int i, int j, int di, int dj, size_t size) {
    if ( size == 0 ) {
